...
const sendNotification = async (client, sub, video) => {
  try {
    ...
    console.log(
      `[YouTube] Posted notification for ${video.id} to #${channel.name}.`
    );
  } catch (notificationError) {
    console.error(
      `[YouTube] Failed to send notification for video ${video.id}:`,
...
async function checkYouTube(client) {
  ...
  try {
    const { documents: allSubs } = await databases.listDocuments(
      DB_ID,
      SUBS_COLLECTION,
      [Query.limit(5000)]
    );
    ...
    for (const sub of allSubs) {
      try {
        ...
        if (!sub.youtubeChannelName) {
          const channelName = await fetchYoutubeChannelName(
          ...
          if (channelName) {
            ...
            needsUpdate = true;
            console.log(
              `[YouTube] Fetched YT channel name for ${sub.youtubeChannelId} to "${channelName}"`
            );
          }
        }
        ...
        if (!sub.discordChannelName) {
          ...
          if (discordChannel) {
            ...
            needsUpdate = true;
            console.log(
              `[YouTube] Fetched Discord channel name for ${sub.discordChannelId} to "#${discordChannel.name}"`
            );
          }
        }
        ...
        if (!feedResponse.ok) {
          console.warn(
            `[YouTube] Failed to fetch feed for ${
              sub.youtubeChannelName || sub.youtubeChannelId
        ...
        if (!sub.announcedVideoIds || sub.announcedVideoIds === "[]") {
          ...
          console.log(
            `[YouTube] Initialized subscription for ${
              sub.youtubeChannelName || sub.youtubeChannelId
            }. Stored ${allVideoIds.length} video IDs.`
          );
          continue;
        }

        const announcedIds = new Set(JSON.parse(sub.announcedVideoIds || "[]"));
        const newVideosToAnnounce = videosInFeed.filter(
          (video) => !announcedIds.has(video.id)
        );

        if (newVideosToAnnounce.length > 0) {
          console.log(
            `[YouTube] Found ${newVideosToAnnounce.length} new video(s) for ${sub.youtubeChannelName}.`
          );
        ...
        console.error(
          `[YouTube] Error processing sub ${sub.$id} (${sub.youtubeChannelName}):`,
          error.message
        );
      }
    }
