...
async function handleAutoMod(message, settings) {
  ...
  // Word Filter
  if (
    autoMod.wordFilterEnabled &&
    Array.isArray(autoMod.wordBlacklist) &&
    autoMod.wordBlacklist.length > 0
  ) {
    const bannedWord = autoMod.wordBlacklist.find((word) =>
      new RegExp(
        `\\b${word.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")}\\b`,
        "i"
  ...
  if (!moderated && autoMod.inviteFilterEnabled) {
    const inviteRegex =
      /(https?:\/\/)?(www\.)?(discord\.(gg|io|me|li)|discordapp\.com\/invite)\/[^\s/]+?(?=\s|$)/i;
  ...
  if (moderated) {
    console.log(
      `[AutoMod] Deleting message from ${message.author.tag} in ${message.guild.name}. Reason: ${reason}`
    );
    message
      .delete()
      .catch((e) =>
    ...
    message.author
      .send(
        `Your message in **${message.guild.name}** was removed because ${reason}.`
      )
      .catch(() => {});
    logAuditEvent(
      message.guild.id,
      "AUTO_MOD_ACTION",
      "AutoMod",
      `Deleted message from ${message.author.tag} because ${reason}.`
    );
  }
...
async function handleLeveling(message, settings) {
  if (
    ...
    settings.leveling.blacklistedChannels.includes(message.channel.id)
  ) {
    return;
  }
  ...
  if (!userCooldown || now - userCooldown > cooldown) {
    ...
    if (userLevelData) {
      ...
      console.log(
        `[XP] Awarded ${xpToAdd} XP to ${message.author.tag} in ${message.guild.name}. Total: ${userLevelData.xp}`
      );
      ...
      if (userLevelData.xp >= xpForNextLevel) {
        ...
        console.log(
          `[LEVEL UP] ${message.author.tag} reached level ${userLevelData.level} in ${message.guild.name}!`
        );
        ...
        const reward = settings.leveling.roleRewards.find(
        ...
        if (reward) {
          try {
            ...
            if (role) {
              ...
              console.log(
                `[LEVEL UP] Awarded role ${role.name} to ${message.author.tag} for reaching level ${userLevelData.level}`
              );
            }
          } catch (e) {
            console.error(
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          // This filter is the fix. It ensures that only objects with the new `date` property
          // are processed, gracefully handling any old data from the previous `day`-based format.
          weeklyData = parsed.filter(
      ...
      // Sort by date and prune to the last 7 entries
      ...
      console.error(
        `Error updating message stats for guild ${message.guild.id}:`,
        error
      );
    }
  }
...
async function handleBuiltInCommands(message, commandName, args) {
  ...
  if (commandName === "help") {
    const builtInCommands = [
      ...
      {
        name: "!reroll <message_id>",
        description: "Rerolls a giveaway winner (Admin only).",
      },
    ];
    ...
    console.log(
      `[CMD] Built-in command '!help' executed by ${message.author.tag} in ${message.guild.name}.`
    );
    return true;
  }
  ...
  if (commandName === "reroll") {
    ...
    try {
      ...
      const giveawayDocs = await databases.listDocuments(
        DB_ID,
      ...
      console.log(
        `[CMD] Built-in command '!reroll' executed by ${message.author.tag} in ${message.guild.name}.`
      );
    } catch (error) {
  ...
  if (commandName === "leaderboard") {
    try {
      ...
      console.log(
        `[CMD] Built-in command '!leaderboard' executed by ${message.author.tag} in ${message.guild.name}.`
      );
    } catch (error) {
      console.error(
  ...
  if (customCommandData) {
    if (customCommandData.isEmbed) {
      try {
        ...
        console.error(
          `Failed to send embed for command !${commandName} in guild ${message.guild.id}:`,
          e.message
        );
        await message.channel.send(
          "Sorry, there was an error displaying this embed command."
        );
      }
    } else {
    ...
    await logCommandUsage(
      message.guild.id,
      `${config.COMMAND_PREFIX}${commandName}`,
      message.author
    );
    console.log(
      `[CMD] Custom command '!${commandName}' executed by ${message.author.tag} in ${message.guild.name}.`
    );
    return true;
  }
  return false;
...
async function onMessageCreate(message) {
  ...
  if (settings.autoMod?.aiEnabled && geminiService) {
    ...
    if (moderationResponse === "FLAG") {
      console.log(
        `[AI Mod] Flagged and deleted message from ${message.author.tag} in ${message.guild.name}.`
      );
      message
        .delete()
        .catch((e) =>
          console.error(`AI Mod failed to delete message: ${e.message}`)
        );
      message.author
        .send(
  ...
  const builtInHandled = await handleBuiltInCommands(
    message,
    commandName,
    args
  );
  ...
  // If we reach here, it was a command-like message that wasn't found.
  console.log(
    `[CMD] Attempted command '!${commandName}' by ${message.author.tag} in guild ${message.guild.id} was not found.`
  );
}
