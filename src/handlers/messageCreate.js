...
async function handleAutoMod(message, settings) {
    ...
    if (moderated) {
        ...
        message.delete().catch(e => console.error(`AutoMod failed to delete message: ${e.message}`));
...
async function handleLeveling(message, settings) {
    if (!settings.leveling?.enabled || settings.leveling.blacklistedChannels.includes(message.channel.id)) {
        return;
    }
    ...
    if (!userCooldown || now - userCooldown > cooldown) {
        ...
        if (userLevelData) {
            ...
            console.log(`[XP] Awarded ${xpToAdd} XP to ${message.author.tag} in ${message.guild.name}. Total: ${userLevelData.xp}`);


            const xpForNextLevel = calculateXpForLevel(userLevelData.level + 1);
            if (userLevelData.xp >= xpForNextLevel) {
                userLevelData.level += 1;
                console.log(`[LEVEL UP] ${message.author.tag} reached level ${userLevelData.level} in ${message.guild.name}!`);
                ...
                if (settings.leveling.channelId) {
                    ...
                    } catch(e) { console.error("Level up message failed:", e.message) }
                }
                
                const reward = settings.leveling.roleRewards.find(r => r.level === userLevelData.level);
                if (reward) {
                    try {
                        const role = await message.guild.roles.fetch(reward.roleId);
                        if (role) {
                            await message.member.roles.add(role);
                            console.log(`[LEVEL UP] Awarded role ${role.name} to ${message.author.tag} for reaching level ${userLevelData.level}`);
                        }
                    } catch (e) { console.error(`Failed to apply role reward ${reward.roleId}:`, e.message) }
                }
            }
...
async function updateMessageStats(message) {
    ...
    if (statsDoc) {
        try {
            // JS's getDay() returns 0 for Sunday, 1 for Monday, etc.
            // Our array is [Mon, Tue, ..., Sun], so Mon is index 0 and Sun is index 6.
            // We need to convert the JS day index to our array index.
            const jsDay = new Date().getDay(); 
            const dayIndex = (jsDay + 6) % 7; // Converts Sun(0)->6, Mon(1)->0, Tue(2)->1, etc.

            const weeklyData = JSON.parse(statsDoc.messagesWeekly || '[]');
            // Ensure the array structure is valid before accessing an index
            if (weeklyData && weeklyData.length === 7 && weeklyData[dayIndex]) {
                ...
                await databases.updateDocument(DB_ID, STATS_COLLECTION, statsDoc.$id, {
                 ...
                 console.error(`[Stats Error] Malformed weeklyData for guild ${message.guild.id}. Data:`, statsDoc.messagesWeekly);
            }
        } catch (error) {
            console.error(`Error updating message stats for guild ${message.guild.id}:`, error);
        }
    }
}
...
async function handleBuiltInCommands(message, commandName, args) {
    ...
    if (commandName === 'reroll') {
        ...
        try {
            ...
            console.log(`[CMD] Built-in command '!reroll' executed by ${message.author.tag} in ${message.guild.name}.`);
        } catch (error) {
    ...
    if (commandName === 'leaderboard') {
        try {
            ...
            console.error(`Error fetching leaderboard for guild ${message.guild.id}:`, error);
    ...
    if (customCommandData) {
        if (customCommandData.isEmbed) {
            try {
                ...
                console.error(`Failed to send embed for command !${commandName} in guild ${message.guild.id}:`, e.message);
...
async function onMessageCreate(message) {
    ...
    if (settings.autoMod?.aiEnabled && geminiService) {
        ...
        if (moderationResponse === 'FLAG') {
            ...
            message.delete().catch(e => console.error(`AI Mod failed to delete message: ${e.message}`));
    ...
    console.log(`[CMD] Attempted command '!${commandName}' by ${message.author.tag} in guild ${message.guild.id} was not found.`);
}
